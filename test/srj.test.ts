/**
 * SimpleRouteJson adapter (task I6): src/srj + api.routeSrj. Covers the mm↔LU mapping, the
 * bounds→Rim / obstacles→Fence·Pour / connections→Pad conversion, the `pcb_trace` output, the
 * differential-pair measurement, and the R-1 invariant on the J802 corpus boards.
 */
...
describe("srj build", () => {
  test("maps a 2-layer board to a Layout", () => {
    ...
    // obstacles became keepouts / held pours.
    expect(layout.fences.length + layout.pours.length).toBeGreaterThan(0);
...
describe("routeSrj on the J802 boards", () => {
  // The J802 corpus boards carry each net's existing copper as net-owned held Pours; most required
  // connections therefore span two Sheets and need a Barrel, which the single-Sheet routing
  // milestone defers (src/route/passes.ts, src/QUESTIONS.md task I4). R-1 must hold regardless, the
  // adapter must produce well-formed output, and any copper it does add must stay within the Rim.
