...
`components[].package` on the 60 boards whose `notes` say so was regenerated by a plain scan of
the board text (rulings Q-I1-27/Q-I1-28, `spec/api/contract.md`), because both references
merge `::n` image variants whose pins coincide and one drops non-Windows-1252 characters from
bare image names (`dsn-dialects.md` D-2, D-23); 1 717 `::n` names and 3 Cyrillic names were
restored to what the `component` scope writes.

Quote characters: `"` is the quote character of every corpus board and `'` is an ordinary
character (`dsn.md` F-4, `dsn-dialects.md` D-11), so the EasyEDA Pro boards (`Issue684-…`,
`Issue721-…`) list a NetGroup named `''` and net-named NetGroups with no members, every net
staying in `default` (D-17); that is what both references read and what the summaries record.
