...
`route()` must reach and the observation on both reference implementations that backs it. The
...
"reference B" are the two sealed reference implementations described in `docs/WALL.md`. Every
table row was produced by running a reference on the board with the settings stated in the file's
header line; where the two references disagree, both observations are given and the ruling is
